...
function parseCommentText(raw: string): {
  quoteAuthor: string;
  quoteText: string;
  bodyText: string;
} | null {
...
function isCanvasComment(comment: PageComment): boolean {
  return /^canvas_\d{1,4}_\d{1,4}$/.test(comment.blockId);
}

function applyCommentEvent(current: PageComment[], detail: PageCommentEventDetail): PageComment[] {
  if (detail.type === 'created') {
    if (current.some((comment) => comment.id === detail.comment.id)) return current;
...
interface CommentItemProps {
  ...
  onNavigateToComment?: (comment: PageComment) => void;
}
...
function CommentItem({
  ...
  const saveEdit = async () => {
    ...
    if (!body) return;
    const fullText = parsed ? `@${parsed.quoteAuthor}: ${parsed.quoteText}\n${body}` : body;
    try {
      const updated = await api.updateComment(comment.id, { text: fullText });
      emitPageCommentEvent({ type: 'updated', comment: updated });
      onUpdate(updated);
      setEditing(false);
    } catch {
      // ignore
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {!editing && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
              ...
              {isCanvasComment(comment) && onNavigateToComment && (
                <button
              ...
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  fontSize: 12,
                  color: comment.resolved ? 'var(--success, #4ade80)' : 'var(--text-muted)',
                  padding: 0,
                }}
...
export default function FloatingComments({
  ...
  const addComment = async () => {
    ...
    if (!text) return;
    try {
      const replyBodyText = replyTo ? parseCommentText(replyTo.text)?.bodyText ?? replyTo.text : null;
      const replyQuote = replyBodyText
        ? `${replyBodyText.slice(0, 60)}${replyBodyText.length > 60 ? '…' : ''}`
        : null;
      const fullText = replyTo && replyQuote ? `@${replyTo.authorName}: ${replyQuote}\n${text}` : text;
      const created = await api.createComment(pageId, {
        text: fullText,
        blockId: replyTo?.blockId || undefined,
      });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      emitPageCommentEvent({ type: 'deleted', pageId, commentId: id });
